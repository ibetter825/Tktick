 ...
 */
    ...
    $.validationEngineLanguage = {
        newLang: function(){
            $.validationEngineLanguage.allRules = {
	            ...
	            //ajax验证菜单编号是否存在
	            "ajaxNickName":{
		    	   	  'url': '/acc/vali?type=nick',
		    	      'alertTextOk': '* 昵称可以使用',
		    	      'alertText': '* 昵称已被使用',
		    	      'alertTextLoad': '* 正在验证,请稍等'
	            }/*,
	            "ajaxDictNo":{
		    	   	  'url': rute.admin+'/dict/vali',  验证程序地址 
		    	      'alertTextOk': '* 编号可以使用',
		    	      'alertText': '* 编号已存在',
		    	      'alertTextLoad': '* 正在验证,请稍等'
	            },
	          //ajax验证角色编号是否存在
	            "ajaxRoleNo":{
		    	   	  'url': rute.admin+'/role/vali',  验证程序地址 
		    	      'alertTextOk': '* 编号可以使用',
		    	      'alertText': '* 角色编号已存在',
		    	      'alertTextLoad': '* 正在验证,请稍等'
	            },
	            "ajaxUserName":{
		    	   	  'url': rute.admin+'/user/vali',  验证程序地址 
		    	      'alertTextOk': '* 用户名可以使用',
		    	      'alertText': '* 用户名已存在',
		    	      'alertTextLoad': '* 正在验证,请稍等'
	            },
	            "ajaxUserPwd":{
		    	   	  'url': rute.admin+'/user/valiPwd',  验证程序地址 
		    	      'alertTextOk': '* 旧密码验证正确',
		    	      'alertText': '* 旧密码输入错误',
		    	      'alertTextLoad': '* 正在验证,请稍等'
	            }*/
		...
		_validateFormWithAjax: function(form, options) {
			...
			$.ajax({
				...
				error: function(data, transport) {
					if(options['onAjaxFormError'])
						options.onAjaxFormError(data, transport);
					else
 ...
 function checkAccount(field, rules, i, options){
 	if(!/^[a-z\d]+(\.[a-z\d]+)*@([\da-z](-[\da-z])?)+(\.{1,2}[a-z]+)+$/.test(field.val())){
 ...
 /**
  * 验证用户昵称，可以下划线，中英文混合，并且不能超过15个字符
  */
 function checkNickName(field, rules, i, options){
	 var num = 0, val = field.val(), rxcn = /[\u4e00-\u9fa5]/;
	 if(!/^(?!_)(?!.*?_$)[a-zA-Z0-9_\u4e00-\u9fa5]+$/.test(val))
	 	return "* 请填写汉字、数字、字母、下划线，不能以下划线开头和结尾";
	 for (var i = 0, j = val.length; i < j; i++) {
         if (rxcn.test(val[i])) num += 2;
         else num += 1;
     }
     if (num > 15) return "* 不能超过15个字符";
 }
 /**
  * 验证18位身份证号码.
  */
